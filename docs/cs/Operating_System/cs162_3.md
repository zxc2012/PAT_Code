...
- Migrating jobs from one CPU to another requires a cost of
invalidating and repopulating caches — so we don’t wish to do this
often
    - **Cache affinity**: try to avoid migration of jobs from one CPU
to another(right)
...
- We don’t wish to leave a CPU idle while another CPU is too busy with all its jobs
    - **Load balancing**: try to keep the workload evenly
distributed
